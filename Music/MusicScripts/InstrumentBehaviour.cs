...
namespace MusicMod
{
    ...
    /// the block XML and read through <see cref="MusicModule"/>; this drives
    /// all of them.
    ///
    ...
    /// </summary>
    public class InstrumentBehaviour : BlockModuleBehaviour<MusicModule>
    {
        ...
        private int DefaultType(List<string> names)
        {
            ...
            Log.Warn("Music: " + Module.Family + " has no instrument called "
        ...
        private void Update()
        {
            ...
            if (mapperShown)
            {
                ...
                if (Time.unscaledTime >= mapperAskAt)
                {
                    ...
                    if (UIF.Available && !MusicPanel.Failed)
                    {
                        ShowInMapper(false);
                    }
                }
            }
            else if (MusicPanel.Failed)
            {
                ShowInMapper(true);
            }
        ...
        private static AudioClip Silence(int rate)
        {
            if (silence == null)
            {
                silence = AudioClip.Create("MusicSilence", 1, 1, rate, false);
        ...
        private void FindVisuals()
        {
            ...
            if (parts.Count == 0)
            {
                ...
                Log.Warn("Music: " + Module.Family + " has no visual of its own to swell.");
            }
